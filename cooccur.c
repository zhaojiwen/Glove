...
int get_cooccurrence() {
    ...
    long long a, a_minus_1, j = 0, k, id, counter = 0, ind = 0, vocab_size, w1, w2, *lookup, *history;
    long long counter2=0; // this counter is added to make sure that we shall save teh data, and free teh memory, before the process be killed.
    int fcount_bigram=10;
    char format[20], filename[200], filename_bigram[200], str[MAX_STRING_LENGTH + 1];
    FILE *fid, *foverflow;
    real *bigram_table, r;
    ...
    while (1) {
        ...
        j++;
        if (counter2>=40000000) { // data has to be saved, and the memory has to be free, otherwise the process will be killed by shell
            long long i=1e6, a=a_minus_1;
            int x,y;
            real r;
            ...
            sprintf(filename_bigram,"%s_%04d.bin",file_head,fcount_bigram);
            fcount_bigram++;
    
            /* Write out full bigram_table, skipping zeros */
            if (verbose > 1) fprintf(stderr, "Writing cooccurrences to disk");
            fid = fopen(filename_bigram,"w");
            
                for (x = 1; x <= vocab_size; x++) {
                if ( (long long) (0.75*log(vocab_size / x)) < i) {i = (long long) (0.75*log(vocab_size / x)); if (verbose > 1) fprintf(stderr,".");} // log's to make it look (sort of) pretty
    ...
    qsort(cr, ind, sizeof(CREC), compare_crec);
    write_chunk(cr,ind,foverflow);
    sprintf(filename,"%s_%04d.bin",file_head,fcount_bigram);
